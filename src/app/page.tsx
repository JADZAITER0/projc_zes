...
const products: Product[] = [
  ...
  const [searchCategory, setSearchCategory] = React.useState("all");
    ...
    const plateKeywords = [" plate", "placca", "rondò"]; // Add more if needed
    const isPlate = (product: Product) =>
      plateKeywords.some((kw) => product.title.toLowerCase().includes(kw));
    ...
    if (searchCategory === "plates") {
      filteredProducts = filteredProducts.filter(isPlate);
    } else if (searchCategory === "modules") {
      filteredProducts = filteredProducts.filter((p: Product) => !isPlate(p));
    }

    const filteredPlates: Product[] = filteredProducts.filter(isPlate);
    const filteredModules: Product[] = filteredProducts.filter((p: Product) => !isPlate(p));
          ...
          {searchCategory === "all" ? (
            <>
              <section className="max-w-6xl mx-auto mb-8">
                <h2 className="text-lg font-bold text-gray-800 mb-2">Plates</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {filteredPlates.map((product, idx) => (
                    <ProductCard
                      key={idx}
                      src={product.src}
                      title={product.title}
              ...
              <section className="max-w-6xl mx-auto mb-8">
                <h2 className="text-lg font-bold text-gray-800 mb-2">Modules</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {filteredModules.map((product, idx) => (
                    <ProductCard
                      key={idx}
                      src={product.src}
                      title={product.title}
